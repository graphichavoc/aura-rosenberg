    ...
    <div class="works">
      <template v-for="item in main.posts">
        <div v-for="image in item.acf.images.filter((e) => e.show_on_frontpage)" class="work">
          <!-- Content -->
          <video v-if='image.video.url' @click='toggleVideo' :src='image.video.url' preload="auto" muted loop ref="videos"></video>
...
export default {
  ...
  methods: {
    ...
    handleScroll(e) {
      ...
      if (!this.$refs.videos || !this.$refs.videos.length) return
